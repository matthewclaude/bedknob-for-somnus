...
- `matthewclaude/bedknob-for-somnus` — this repo. The source, the CI, the release workflow, the CHANGELOG. Public since 2026-09-10, source-available (PolyForm NC). It holds **no Releases** and serves **no Pages site**. (It does carry a `gh-pages` branch, but that branch is inherited from the upstream fork: twelve `deploy:` commits ending 2026-08-05, whose tree is the upstream project's flasher page and its `firmware/latest/orion-dial-merged.bin` / `firmware/beta/orion-dial-merged.bin`. Nothing Somnus has ever been deployed there. See §4(b) and §8.)
...
- **(b) A second `peaceiris/actions-gh-pages` deploy** to *this* repo's `gh-pages` branch: `github_token: ${{ secrets.GITHUB_TOKEN }}`, no `external_repository`, no `personal_token`, the same `publish_dir: ./_site`, `publish_branch: gh-pages`, `keep_files: true`. This is what makes the new Pages site exist. Two things to know about the target branch. First, it already exists on `origin` and is the upstream fork's: its tree is the upstream flasher page plus `firmware/latest/orion-dial-merged.bin` and `firmware/beta/orion-dial-merged.bin`, last deployed 2026-08-05. `keep_files: true` will overwrite `index.html` and the two manifests but leave the two `orion-dial-merged.bin` files in place as dead weight. They are harmless (nothing links to them) but should be removed once, by hand, either before the first deploy or in the cut-over; the spec does not propose `force_orphan` for the same reason the existing comment (lines 180–184) rejects it — it would wipe the other channel's image on every deploy. Second, **an owner-only prerequisite before the first dual-published tag: enable GitHub Pages on `bedknob-for-somnus`, source = `gh-pages` branch, root.** Nothing in the workflow can do that; a deploy to a branch that Pages is not serving publishes nothing.
...
6. Remove the two stale `orion-dial-merged.bin` files from this repo's `gh-pages` branch if not already done (§4(b)).
