...
4. **Night face at STANDBY.** Force night from Settings > Timezone by selecting a listed zone whose local time falls inside the night window. The on-device picker is a curated list of eleven zones (`DIAL_TZ_IANA`, `dial_state.h`) and `Asia/Tokyo` is not among them — the original Tokyo trick ran through the provisioning web page, which accepts any IANA zone a browser reports. From US Central in the evening, UK or Central Europe is the zone that lands inside a 21:00–07:00 window. Restore the real zone afterwards. Confirm the number/water alternation still runs at the dim floor and that the water number tracks. The alternation is an LVGL timer at 2 s (`ALT_SETPOINT_MS`/`ALT_WATER_MS` in `scr_dial.c`) and the poll cadence cannot freeze it; what 300 s can do is leave the WATER number up to five minutes stale, and that staleness is what this item measures. `alt_want` additionally requires the side on and HEATING or COOLING (setpoint and water more than 0.5 °C apart, `dial_zone_kind`), so the bench must hold the pad off its target for the whole observation or the timer is correctly deleted and the face correctly stops alternating.
