...
static void apply_palette_and_state(const app_state_t *st)
{
    ...
    // temp_c-vs-actual_c comparison dial_zone_kind used just above; see
    // ui_screens_internal.h). Somnus's local API has no thermal-relief
    // ("boost") concept and no server-side sleep schedule to report a
    // hold/until grammar from (both were Orion get_device_state/
    // get_sleep_schedules fields) — removed along with the rest of that
    // pipeline (see components/dial_somnus) — so the pill is now just this
    // one computed status, and only shown while the zone is actually on;
    // an off zone has nothing to hold or drive toward.
    ...
    // Power button — the PRIMARY (and, since the boost controls' removal,
    // now the ONLY) control on this face — always carries a clearly visible
    // border. Off: ink_secondary (visible, but quieter than the on state's
    // accent). On: accent, unchanged.
    ...
    // field's comment in dial_state.h), so this never actually shows on a
...
static void power_event_cb(lv_event_t *e)
{
    ...
    // asking it to toggle: the worker commits only after the write to the pad
    // returns, so deriving the new value there meant the face didn't move until
    // an HTTP round trip had completed — a button that visibly did nothing for
    // a second. The next poll reconciles, and reverts this if the write failed.
    app_state_t st;
    dial_state_get(&st);
...
static void create(lv_obj_t *scr, void *arg)
{
    ...
    lv_obj_move_foreground(s_handle);   // stays topmost so it always wins the drag over the arc/level beneath it
    ...
    // Purely informational now (no thermal-relief cancel action left to tap
    // — see apply_palette_and_state): CLICKABLE stays cleared for good
    // (lv_obj_create's own default is ON — this undoes that) so a tap on the
    // pill falls through to whatever's underneath.
    ...
    // #11 Power disc — the ONLY control on this row. Short tap toggles power
    // (CLICKED); long-press opens the Schedule/Hold picker (LONG_PRESSED,
    // §2) — see power_long_press_cb for how the two are kept from BOTH
    // firing on one long press. 72px is this project's own >=72px
    // round-screen touch floor; left unpadded (no ext_click_area) so it
    // doesn't eat into the "Update available" label's own clearance just
    // below it (the tighter budget in this row — see that label's own
    // comment).
    ...
    // Away badge — tiny, dormant (see app_state_t.away's comment; this never
    // actually shows on a Somnus dial today, but costs nothing to keep
    // rendering against the day it does).
...
//
// The Schedule/Hold picker opens via long-press on the power disc, not a
// swipe on the dial itself (see power_long_press_cb) — up/down are simply
